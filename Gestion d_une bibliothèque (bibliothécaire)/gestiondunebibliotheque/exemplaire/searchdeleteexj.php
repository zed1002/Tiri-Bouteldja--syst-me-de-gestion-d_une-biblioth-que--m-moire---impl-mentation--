...
<?php include "../book/navigation.html"?>
  ...
  <form  method="POST">
      ...
      <div class="searchbar-container">
          <h1 class="heading-primary heading-primary--main"> EDIT/DELETE JOURNAL COPIES </h1>
...
<?php
	...
	if (isset($_POST['srchusrbtn'])):
		...
		if ( ($column == "" ||$column != "refr_exp" && $column != "emp" && $column != "annee" && $column != "ref_fk")){
    //default column
      $column = "refr_exp";}
      if ($q==""){
echo "empty field";
...
if($q!=""):
        $sql = $bdd->query("SELECT * FROM exemplairej WHERE $column LIKE '%$q%'"); ?>
  ...
  <table class="table">
    ...
    <td><?php echo $row['refr_exp'];?></td>
      ...
      <td>
        ...
        <a href="updateexemplairej.php?edit=<?php echo $row['refr_exp']; ?>" class="btn btn-info" name='edit'>Edit</a>
        
        <?php
        $ref=$row['refr_exp'];
        $query = "SELECT * FROM emprunt
          where ref_expe='$ref' and genre='revue'";
$result = mysqli_query($bdd,$query);
if(mysqli_num_rows($result) == 0): ?>

        <a class="btn btn-info" name="delete" value="delete"  onClick="crnexej(<?php echo $row['refr_exp']; ?> ,<?php echo $row['ref_fk']; ?>)" > Delete </a>
