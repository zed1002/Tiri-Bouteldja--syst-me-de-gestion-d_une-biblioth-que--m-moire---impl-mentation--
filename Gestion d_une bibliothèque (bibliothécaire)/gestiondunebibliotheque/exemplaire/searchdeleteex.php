...
<?php include "../book/navigation.html"?>
  ...
  <form  method="POST">
      ...
      <div class="searchbar-container">
          <h1 class="heading-primary heading-primary--main"> EDIT/DELETE BOOK copies </h1>
  ...
  <div class="select" >
      ...
      <option value=""> &#xf0b0; select filter </option>
       ...
       <option value="ref_fk">ref_book</option>
...
<?php
	...
	if (isset($_POST['srchusrbtn'])):
		...
		if ( ($column == "" ||$column != "ref_exp" && $column != "titre" && $column != "auteur" && $column != "edi" && $column != "emp" && $column != "annee" && $column != "ref_fk")){
    //default column
      $column = "ref_exp";}
      if ($q==""){
echo "emptu field";
  ...
  <table class="table">
    ...
    <td><?php echo $row['ref_exp'];?></td>
      ...
      <td>
        ...
        <a href="updateexemplaire.php?edit=<?php echo $row['ref_exp']; ?>" class="btn btn-info" name='edit'>Edit</a>
       <?php 
       $ref=$row['ref_exp'];
        $query = "SELECT * FROM emprunt
          where ref_expe='$ref' and genre='livre'";
$result = mysqli_query($bdd,$query);
if(mysqli_num_rows($result) == 0): ?>

        <a class="btn btn-info" name="delete" value="delete"  onClick="crnexe(<?php echo $row['ref_exp']; ?> ,<?php echo $row['ref_fk']; ?>)" > Delete </a>
<?php endif;
      if(mysqli_num_rows($result) > 0): 
